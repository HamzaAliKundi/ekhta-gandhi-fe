    ...
    return (
        <div className="w-full bg-gray-50 flex justify-center">
            <div className="w-full max-w-[1440px] bg-gray-50 relative overflow-hidden">
                ...
                {/* Background Pattern - Similar to header but more subtle */}
                        ...
                        for (let row = 0; row < 12; row++) {
                            for (let col = 0; col < 20; col++) {
                                boxes.push(
                                    <div
                                        key={`${row}-${col}`}
                                        className="absolute border border-dashed border-gray-400 rounded-lg"
                                        style={{
                                            width: '70px',
                                            height: '70px',
                                            top: `${row * 80}px`,
                ...
                <div className="absolute inset-0 opacity-3 lg:hidden">
                    {(() => {
                        const boxes = [];
                ...
                {/* Main Content */}
                <div className="w-full flex flex-col lg:flex-row items-center justify-between px-4 sm:px-6 lg:px-12 py-12 lg:py-20 relative z-10 gap-8 lg:gap-12">
                    
                    {/* Left Section - Image and Title */}
                    <div className="w-full lg:w-1/2 flex flex-col gap-6">
                        {/* Title */}
                        <h2 className="font-poppins font-medium text-black text-[51px] leading-[100%] tracking-[0%]">
                        ...
                        <div className="relative">
                            <img 
                                ...
                                className="w-[577px] h-[727px] object-cover rounded-2xl shadow-lg mx-auto lg:mx-0"
                            />
                        </div>
                    </div>
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col gap-6 lg:gap-8">
                        {features.map((feature, index) => (
                            <div 
                                key={index}
                                className="bg-white rounded-[20px] p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300 w-[680px] h-[216px]"
                            >
                                <h3 className="text-xl sm:text-2xl font-semibold text-black mb-3 font-poppins">
                                    {feature.title}
                                </h3>
